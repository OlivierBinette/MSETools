...
-   The **analyses** folder contains the analyses and figures for Binette
    ...
    using SLURM. To run the entire analysis from scratch, use the command `make clear_cache && srun make all` from within the analyses folder. Long-running programs on a cluster can be run within a detachable terminal (e.g. `tmux`) to avoid connection issues.
